...
export const CurrencyConverter = ({ API_KEY }) => {
  ...
  const fetchCurrencies = useCallback(async () => {
    ...
    try {
      ...
      setError(
        `Error al cargar las monedas: ${err.message}. Asegúrate de que tu clave API sea válida.`
      );
    } finally {
      setLoading(false);
    }
  ...
  const convertCurrency = useCallback(async () => {
    ...
    try {
      const response = await fetch(
        `${BASE_URL}/latest?apikey=${API_KEY}&base_currency=${fromCurrency}&currencies=${toCurrency}`
      );
      if (!response.ok) {
        throw new Error(`HTTP error! status: ${response.status}`);
      }
      ...
      if (data.data && data.data[toCurrency]) {
        ...
        setError(
          "No se pudo obtener la tasa de cambio para las monedas seleccionadas."
        );
        setConvertedAmount(null);
      }
    } catch (err) {
      ...
      setError(
        `Error al convertir: ${err.message}. Verifica tu clave API y las monedas.`
      );
      setConvertedAmount(null);
    } finally {
      setLoading(false);
    ...
    <div className="bg-white p-8 rounded-xl shadow-2xl w-full max-w-md">
      ...
      {API_KEY === "YOUR_API_KEY_HERE" && (
        ...
        >
          ...
          <p>
            Por favor, reemplaza 'YOUR_API_KEY_HERE' en el código con tu clave
            API de FreecurrencyAPI.com para que la aplicación funcione.
          </p>
        </div>
      )}
      ...
      {convertedAmount !== null && !loading && !error && (
        <div className="text-center mt-6 p-4 bg-blue-100 rounded-lg shadow-inner">
          <p className="text-xl text-blue-800 font-bold">
            {amount} {fromCurrency} ={" "}
            <span className="text-2xl">{convertedAmount}</span> {toCurrency}
          </p>
        </div>
      )}
    </div>
